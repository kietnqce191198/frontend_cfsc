...
function LoyaltyPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (!token) {
                setLoadError("Please log in to view your membership program.");
                return;
            }
            ...
            if (!customerId) {
                setLoadError("No membership account was found for this user.");
                return;
            }
            ...
            setLoadError("Membership data is unavailable right now. Please try again later.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const filteredPoints = points.filter(
        p => (!filterType || p.type === filterType) && isDateInRange(p.createdAt)
    );
    const filteredRedemptions = redemptions.filter(r => isDateInRange(r.createdAt));
    const filteredTierHistory = tiers.filter(t => isDateInRange(t.changedAt));
            ...
            <section className="account-page-shell">
                <div className="account-container-center">
                    <div className="account-card account-card-clean">
                        ...
                        {tab === "info" && isLoading && renderEmptyState(
                            "fas fa-spinner fa-spin",
                            "Loading membership data",
                            "Please wait while we load your loyalty overview."
                        )}
                        {tab === "info" && !isLoading && loadError && renderEmptyState(
                            "fas fa-circle-exclamation",
                            "Membership data unavailable",
                            loadError
                        )}
                        {tab === "info" && !isLoading && !loadError && !loyalty && renderEmptyState(
                            "fas fa-gem",
                            "No membership data yet",
                            "Your account does not have loyalty information yet."
                        )}
                        {tab === "info" && !isLoading && !loadError && loyalty && (
                            ...
                            <div className="history-section">
                                {isLoading
                                    ? renderEmptyState(
                                        ...
                                        "Please wait while we load the membership tiers."
                                    )
                                    : allTiers.length === 0
                                        ? renderEmptyState(
                                            "fas fa-layer-group",
                                            "No tiers available",
                                            "Tier and benefit information has not been configured yet."
                                        )
                                        : (
                                ...
                                <div className="loyalty-table">
                                    {isLoading
                                        ? renderEmptyState(
                                            ...
                                            "Please wait while we fetch your loyalty activity."
                                        )
                                        : filteredPoints.length === 0
                                            ? renderEmptyState(
                                                "fas fa-receipt",
                                                points.length === 0 ? "No points history yet" : "No matching point activity",
                                                points.length === 0
                                                    ? "You have not earned or redeemed any points yet."
                                                    : "Try changing the filters to see more point activity."
                                            )
                                ...
                                <div className="loyalty-table">
                                    {isLoading
                                        ? renderEmptyState(
                                            ...
                                            "Please wait while we load your redeemed rewards."
                                        )
                                        : filteredRedemptions.length === 0
                                            ? renderEmptyState(
                                                "fas fa-gift",
                                                "No redeemed rewards yet",
                                                "Rewards you redeem will appear here."
                                            )
                                ...
                                <div className="loyalty-table">
                                    {isLoading
                                        ? renderEmptyState(
                                            ...
                                            "Please wait while we load your membership changes."
                                        )
                                        : filteredTierHistory.length === 0
                                            ? renderEmptyState(
                                                "fas fa-chart-line",
                                                "No tier changes yet",
                                                "Your membership tier history will appear here after an upgrade or downgrade."
                                            )
                                            : filteredTierHistory.map((t, i) => (
                                                <div key={i} className="loyalty-row">
                                                    <div className="row-left">
