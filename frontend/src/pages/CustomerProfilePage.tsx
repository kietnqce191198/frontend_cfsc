...
const CustomerProfilePage = () => {
  ...
  const renderHeroActions = () => {
      ...
      return (
        <>
          <button type="button" className="customer-profile-action-btn" onClick={() => navigate("/admin/users")}>
            <i className="fas fa-arrow-left"></i><span>Back to User Directory</span>
          </button>
        </>
      );
    }
    return (
      <>
        <Link to="/" className="customer-profile-action-btn secondary"><i className="fas fa-house"></i><span>Storefront</span></Link>
        ...
        <Link to="/account" className="customer-profile-action-btn"><i className="fas fa-pen"></i><span>Edit Profile</span></Link>
      </>
    );
  };
    ...
    <div className={`admin-container customer-profile-shell ${isAdminContext ? "customer-profile-admin" : "customer-profile-public"}`}>
      {isAdminContext && (
        <div className="customer-profile-toolbar">
          <button type="button" className="create-btn" onClick={() => navigate("/admin/users")}>Back to User Directory</button>
        </div>
      )}

      {isLoading ? (
        <div className="loading-state"><i className="fas fa-spinner fa-spin"></i><span>Loading profile...</span></div>
      ) : message && message.type === "error" && !profile ? (
        <div className="error-card"><h3>Unable to load profile</h3><p>{message.text}</p></div>
      ) : profile ? (
        <>
          {message && (
          ...
          <section className="customer-profile-grid customer-profile-grid-main">
            ...
            <article className="customer-profile-card">
              ...
              <p className="customer-profile-address-note">This is the primary address synced from the account profile in auth-service.</p>
            </article>
          </section>

          <section className="customer-section">
            <div className="customer-section-head">
              <div><h3>Order Summary</h3><p>A quick snapshot of this customer's order activity.</p></div>
            </div>
          ...
          <section className="customer-section">
            <div className="customer-section-head">
              <div><h3>Loyalty Status</h3><p>Current loyalty tier and available points.</p></div>
            </div>
            {profile.loyalty ? (
              <div className="customer-profile-grid customer-profile-grid-tight">
                ...
                <article className="customer-profile-card">
                  <h3>Available Points</h3><div className="customer-profile-highlight">{profile.loyalty.current_points ?? 0}</div>
                </article>
              </div>
            ) : (<div className="customer-empty">No loyalty data is available for this customer.</div>)}
          </section>
          ...
          <section className="customer-section">
            <div className="customer-section-head customer-section-head-split">
              <div>
                ...
                <p>Delivery addresses from customer-service. Each customer can save up to 5 addresses.</p>
              </div>
              {!isAdminContext && (
                <button type="button" className="customer-address-add-btn" onClick={handleOpenAddAddressModal}>
                  <i className="fas fa-plus"></i><span>Add Address</span>
                </button>
              )}
            </div>
            ...
            <div className="customer-address-modal-backdrop">
              <div className="customer-address-modal">
                <div className="customer-address-modal-head">
                  <div><h3>Add Address</h3><p>Create another delivery address for this account.</p></div>
